...
class Leaderboard extends Command {
	...
	async run(message, args) {
		const isOnlyOnMobile = message.author.presence.clientStatus
			? JSON.stringify(Object.keys(message.author.presence.clientStatus)) ===
			  JSON.stringify(['mobile'])
			: false;

		const type = args[0];
		if (!type || (type !== 'money' && type !== 'level' && type !== 'rep')) {
			return message.error('economy/leaderboard:MISSING_TYPE');
		}

		if (type === 'money') {
			const members = await this.client.usersData.find().lean(),
				membersLeaderboard = members
					...
					.filter(function(u) {
						if (u.id && u.value && !isNaN(parseInt(u.value))) return true;
					})
					.sort((a, b) => b.value - a.value);
			const table = new AsciiTable('LEADERBOARD');
			table.setHeading(
				'#',
				message.translate('common:USER'),
				message.translate('common:CREDITS')
			);
			...
			message.channel.send(`\`\`\`\n${newTable.toString()} \`\`\`\n${isOnlyOnMobile ? message.translate('economy/leaderboard:MOBILE') : ''}`);
		} else if (type === 'level') {
			const members = await this.client.usersData.find().lean(),
				membersLeaderboard = members
					.map(m => {
			...
			table.setHeading(
				'#',
				message.translate('common:USER'),
				message.translate('common:LEVEL')
			);
			if (membersLeaderboard.length > 20) membersLeaderboard.length = 20;
			const newTable = await fetchUsers(membersLeaderboard, table, this.client);
			message.channel.send(`\`\`\`\n${newTable.toString()} \`\`\`\n${isOnlyOnMobile ? message.translate('economy/leaderboard:MOBILE') : ''}`);
			...
			table.setHeading(
				'#',
				message.translate('common:USER'),
				message.translate('common:POINTS')
			);
			...
			return new Promise(resolve => {
				...
				array.forEach(element => {
					client.users.fetch(element.id).then(user => {
						...
						table.addRow(index++, username, element.value.commas());
					});
				});
				resolve(table);
			});
