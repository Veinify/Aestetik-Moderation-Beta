...
class snake {
  ...
  renderBoard() {
    ...
    for (let i = 0; i < this.snakePos.length; i++) {
      ...
      else board[pos.y][pos.x] = this.snake;
    }
  ...
  die(collector) {
    collector.stop();
    this.start.reactions.removeAll();
    this.start.inlineReply(
  ...
  initCol(message) {
    ...
    const collector = this.start.createReactionCollector(filter, {time: 600000});
      ...
      switch (reaction.emoji.name) {
        case "⬆️":
          ...
          if (this.checkSnake(this.startBoard, Head.x, Head.y - 1))
            return this.die(collector);
          if (this.checkFood(this.startBoard, Head.x, Head.y - 1))
            this.addLength("up");
          this.move("up");
          if (Head.y - 1 < 0) return this.die(collector);
          break;
        case "⬇️":
          reaction.users.remove(message.author.id)
          if (this.checkSnake(this.startBoard, Head.x, Head.y + 1))
          ...
          if (this.checkFood(this.startBoard, Head.x, Head.y + 1))
            this.addLength("down");
          this.move("down");
          if (Head.y + 1 > this.startBoard.length - 1) return this.die(collector);
          break;
        case "⬅️":
          reaction.users.remove(message.author.id)
          ...
          if (this.checkFood(this.startBoard, Head.x - 1, Head.y))
            this.addLength("left");
          this.move("left");
          if (Head.x - 1 < 0) return this.die(collector);
          break;
        case "➡️":
          reaction.users.remove(message.author.id)
          if (this.checkSnake(this.startBoard, Head.x + 1, Head.y))
          ...
          this.move("right");
          if (Head.x + 2 > this.startBoard[0].length) return this.die(collector);
          break;
        case '⏹️':
          reaction.users.remove(message.author.id)
          return this.die(collector);
